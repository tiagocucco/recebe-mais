create or replace package body rmais_process_pkg_test as
  --
    procedure ins_log (
        p_msg varchar2
    ) is
    ...
    begin
      --
        select
            ...
            catalog_code_ncm,
            item_type,
            xxrmais_util_v2_pkg.get_filial(item_code_efd, p_cnpj_tomador)
        into
            p_item_code_efd,
            p_item_descr_efd,
            p_uom,
  ...
  --
    ...
    --
        l_body := '{"cnpj_fornecedor" : "'
                  ...
                  || '"}';
    --
        l_response := json_value(rmais_process_pkg_test.get_response(l_url, l_body),
           '$.DATA.ITEM_ORACLE');
    --
        return l_response;
    --
  ...
  --
    ...
    --
      ...
      --
            r_issuer.info := rmais_process_pkg_test.get_taxpayer(p_fornec, 'ISSUER');
            r_issuer.docs := rmais_process_pkg_test.get_po_list(l_body_po);
            r_issuer.cnpj := p_fornec;
            r_issuer.receiver := p_receiv;
      --
            rmais_process_pkg_test.ins_issuer(r_issuer);
      --
            setpo;
      --
        end if;
  ...
  --
    ...
    --
      ...
      --
            r_issuer.info := rmais_process_pkg_test.get_taxpayer(p_fornec, 'ISSUER');
      --
            l_transaction_id := rmais_process_pkg_test.get_po_list_v2(l_body_po);
      --
            begin
        --
            ...
            l_body :=
                json_object(
                    'P_WITHHOLD_TAX_CLASSIF_CODE' value
                        case
                            when l_withhold = 'ISS_RET_2.00' then
                                'ISS200'
                            when l_withhold = 'ISS_RET_2.01' then
                                'ISS201'
                            else
                                l_withhold
                        end,
  ...
  --
    ...
    */
    -- Adicionado por Robson em 24/01/2023
        r$po := rmais_process_pkg_test.cursor_po_line(p_transaction_id => p_transaction_id);
    --
    ...
    --
        for r in (
           ...
           --, NVL(l.item_info.DESCRIPTION,l.item_descr_efd) item_desc
                    ...
                    nvl(
                        ...
                        rmais_process_pkg_test.get_parameter('DETERM_FACTOR')
                    )                                                                                                                                         determining_factor
                    ,
                    l.catalog_code_ncm,
                    l.cfop_to,
           ...
           --,'BusinessUnit'                        IS nvl(h.receiver_info.DATA.BU_NAME, h.receiver_name)
                                ,
                                'BusinessUnit' is nvl(h.receiver_info.data.bu_name,
                                                      'HDI Seguros S.A. BU')
       --    ,'Supplier'                            IS coalesce(h.issuer_info.DATA.PARTY_NAME,(
         ...
         --  SELECT json_arrayagg(json_object (
          ...
          --,'PayGroup'                             IS CASE WHEN nvl(h.PAYMENT_INDICATOR,'N') = 'N' THEN '' ELSE nvl(RMAIS_PROCESS_PKG_TEST.Get_Parameter('GET_NO_PAY'),'NAO GERA PAGAMENTO') END --21_09_2021 adicionado nao gera pagamento
                                ,
...
99	Outros
    ...
    */
                            ...
                            select
                                json_arrayagg(
                                    json_object(
                                        ...
                                        'isvSubserie' is '',
                                        'isvAccessKey' is h.access_key_number
                                    )
                                )
                            from
                                dual
           ...
           , 'ProductFiscalClassificationType'    IS nvl(l.CATALOG_CODE_NCM,'LACLS_NCM_SERVICE_CODE')*/,
                                                ...
                                                'UserDefinedFiscalClassification' is l.user_defined,
             ...
             --'DueDate'                    IS to_char(nvl(first_due_date,sysdate),'RRRR-MM-DD'),--json_value(l2.order_info,'$.LINE_LOCATIONS.NEED_BY_DATE'),--to_char(first_due_date,'RRRR-MM-DD'),
                                            ...
                                            where
                                                ...
                                                'PaymentMethod' is decode(
                                            to_char(length(h.boleto_cod)),
                                            '47',
                                            'BOLETO',
                                            '48',
                                            'CONCESSIONARIAS',
          ...
          --chamada para ws base64 de layout via xml
            ...
            --
                    declare
                        ...
                        l_url  varchar2(300) := 'http://144.22.170.155/luznfe/rest.php?class=getPDF'
                                               || chr(38)
                                               || 'method=executar';
            --UTL_URL.Escape(RMAIS_PROCESS_PKG_TEST.Get_Parameter('URL_GENERATE_PFD','TEXT_VALUE'), True);--'http://150.230.68.115/luznfe/rest.php?class=getPDF&method=executar'; --parametrizar endereço
                    begin
                        l_body :=
                            json_object(
              ...
              --chamada WS para gerar report
                  ...
                  --
                            declare
                                ...
                                l_body clob;
                                l_url  varchar2(300) := 'http://144.22.170.155/luznfe/rest.php?class=getPDF&method=executar'; --parametrizar endereço
                            begin
                                l_body :=
                                    json_object(
                                        'transaction_id' value l_transaction_id,
  ...
  --
    ...
    --
        rmais_process_pkg_test.insert_ws_info(
              ...
              --
                ...
                --
                    ...
                    /*Inclusão erickson send boleto 08/03/2023*/
                                    if l_model in ( '06', '21', '22', '28', '29' ) then
  ...
  --
    function get_registrationid (
        ...
        l_url  varchar2(1000) := rmais_process_pkg_test.get_parameter('GET_TAX_REGISTRATION')
                                || '/';
        l_resp clob;
    begin
    --
        ...
        l_resp := rmais_process_pkg_test.get_response(l_url);
  ...
  --
    ...
    --
      ...
      --
        ...
        --
                l_transaction_id := rmais_process_pkg_test.set_transaction_po_arrays(r.issuer_document_number,
                                                                                     get_bu_cnpj(r.receiver_document_number),
                                                                                     l_transaction_id);
        --
        --
            end if;
      --l_transaction_id := RMAIS_PROCESS_PKG_TEST.set_transaction_po_arrays(r.issuer_document_number,r.receiver_document_number,l_transaction_id);
      --
    ...
    --END IF;
     --
          ...
          --
            ...
            --
                        get_item_erp(r_efd.rhea.issuer_document_number,
                                     ...
                                     r_efd.rlin(ix_l).rlin.account_cc);
            --
                    end if;
          --
                end if;
          ...
          --
            ...
            --
                  ...
                  --Inclusão de combinação contábil
                            r_efd.rlin(ix_l).rlin.account_cc := xxrmais_util_v2_pkg.get_filial(r_efd.rlin(ix_l).rlin.item_code_efd,
                                                                                               r_efd.rhea.receiver_document_number);
                  --
                            l_id := rmais_ws_info_s.nextval;
                  --
                            rmais_process_pkg_test.insert_ws_info(l_id, 'GET_ITEM');
                  --
                            l_transaction_id2 := rmais_process_pkg_test.get_itens(l_id,
                                                                                  r_efd.rlin(ix_l).rlin.item_code_efd,
                                                                                  '');
                  --
      ...
      --END LOOP;
        ...
        --
             ...
             BEGIN
               --
               l_r := RMAIS_PROCESS_PKG_TEST.CHECK_TAX_LINE(r_Efd.rLin(ix_l).rLin.efd_line_id);
               --
               IF l_r = 'F' THEN
                 --
     ...
     /* IF XXRMAIS_UTIL_V2_PKG.get_destination(r_Efd.rHea.efd_header_id) ='EXPENSE' THEN
      ...
      --
          ...
          --  declare
              ...
              --
                ...
                begin
                    print('Robson - 1a: ' || substr(r_efd.rhea.issuer_info,
                                                    1,
                                                    2));

                exception
                    when others then
                        print('Robson - 2a Erro');
                end;
            ...
            begin
                begin
                    print('Robson - 1b: ' || substr(r_efd.rhea.issuer_info,
                                                    1,
                                                    2));
                exception
                    when others then
                        print('Robson - 2b Erro');
                end;
        --
      ...
      --
            if
                not g_po_find
                and l_defined_role = 'MT'
                and l_role = 'item_default'
      ...
      l_role         := null; then*/
        ...
        --
                ...
                print('Parametro: ' || nvl(
                    rmais_process_pkg_test.get_parameter('SEND_ERP_AUTO'),
                    '2'
                ));
...
--
  ...
  --
        print('l_body: ' || l_body);
  --
        l_response := rmais_process_pkg_test.get_response2(l_url, l_body, 'POST');
  --
...
--
    procedure update_invoice (
        ...
        l_url_mod    constant varchar2(1000) := 'http://152.67.41.84:9000/api/payables/v2/updateInvoiceModel';
    ...
    --
        with tp_efd as (
            select
                ...
                nvl(a.receiver_info.data.bu_name,
                    'HDI Seguros S.A. BU')                        businessunit,--'INATIVO-HDI Seguros S.A. BU' BusinessUnit,
        ...
        l_response := rmais_process_pkg_test.get_response2(l_url_mod, l_body_doc, 'POST');
    --
    ...
    --
        ...
        --  
            l_response := rmais_process_pkg_test.get_response2(l_url_att, l_body_att, 'POST');
        --
            print('l_response att' || substr(l_response, 1, 2000));
        --
        ...
        set
            a.document_status =
                case
                    when l_flag_falha is null then
                        a.document_status
...
--
    ...
    begin
        for c in (
            with tp_proc_ctrl as (
                select
                    b.efd_header_id
                from
                    rmais_reprocess_ctrl b
                where
                    b.flag_reprocess_ctrl = 1
            )
            select
                a.efd_header_id
            from
            ...
            rmais_process_pkg_test.update_invoice(p_header_id => c.efd_header_id);
        end loop;
...
--
    function cancel_nf_erp (
        ...
        l_url_mod  constant varchar2(1000) := 'http://152.67.41.84:9000/api/payables/v2/cancelInvoiceService';
    ---l_url_att constant varchar2(1000) := 'http://152.67.41.84:9000/api/payables/v2/updateInvoiceAttach';
        l_response varchar2(4000);
        l_body_doc clob;
        l_body_att clob;
...
-- sqlcl_snapshot {"hash":"defd78c4d46e2da6397c6e322a22abfdcce68ab2","type":"PACKAGE_BODY","name":"RMAIS_PROCESS_PKG_TEST","schemaName":"SCH_RMAIS_DEV","sxml":""}