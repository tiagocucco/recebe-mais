create or replace package body xxrmais_util_v2_pkg as --CLL_F369_efd_send_ri_pkg
...
-- -----------------------------------------------------------------------------------
  ...
  --
    function get_access_key_number (
        ...
        l_numero_nff rmais_efd_headers.access_key_number%type;
    begin
 ...
 --
  ...
  ----------------------------------------------
        procedure load_read_file_xml_nfse (
            psource    clob,
            p_filename in rmais_efd_headers.blob_filename%type default null,
            p_file     in rmais_efd_headers.blob_file%type default null,
            p_process  in rmais_efd_headers.poc%type
        ) is
    --
            vidx              number;
            clin              c$refcur;
      ...
      --
       ...
       -- <?xml version="1.0" encoding="ISO-8859-1"?>
        ...
        --
                        ...
                        l_source_trat := replace(l_source_trat, '<?xml version="1.0" encoding="ISO-8859-1"?><NFe', '<?xml version="1.0" encoding="UTF-8"?><NFe'
                        );
  ...
  --SELECT * FROM all_directories WHERE directory_name =  'RMAIS_ERROR_18182137000170'
    ...
    --    l_filename rmais_efd_headers.BLOB_FILENAME%type;
            l_file               rmais_efd_headers.blob_file%type;
            vidx                 number;
            l_ctrbr              number;
    --
            l_xml                clob;
          ...
          --
           ...
           '$.DATA.BU_NAME');
            ...
            --
              ...
              --
                            insert into rmais_efd_headers values l_header returning efd_header_id into l_id_header;
              --
                        else
              --
            ...
            /*  rSource.rLines(vIDX).Reg.Ipi_Amount_Recover        := 0;
              ...
              /*rSource.rLines(vIDX).Reg.Icms_Base                 := NVL(rLin.Icms_Base, CASE WHEN rSource.rLines(vIDX).Reg.Icms_Tax > 0 THEN NVL(rLin.Valor_Tot,0) ELSE 0 END);
                ...
                --rSource.rEfd.org_id                                := rSource.rCtrl.Operating_unit;
                                ...
                                l_lines.last_update_date := sysdate;
                                l_lines.last_updated_by := -1;
                                update rmais_efd_headers a -- Robson (15/03/2023) start
                                set
                                    a.iss_ret_flag =
                                        case
    ...
    --base64decode_to_blob(json_value(l_clob,'$.BASE64' returning clob))
      ...
      --
        ...
        --
                ...
                l_file := base64decode_to_blob(json_value(p_source_orig, '$.file' returning clob));
        --
            end if;
...
--
    function get_link_nfse (
        ...
        l_cod     rmais_efd_headers.cod_verif_nfs%type;
        l_num     rmais_efd_headers.document_number%type;
            ...
            exception
                ...
                --
                    ...
                    set
                        ...
                        bu_code = reg.bu_name
                    where
                        lpad(cnpj, 14, '0') = lpad(reg.registration_number_le, 14, '0');
                --
                when others then
                --
...
-- sqlcl_snapshot {"hash":"734f6983402aa6458016bdcb5bc279dd07b8bc21","type":"PACKAGE_BODY","name":"XXRMAIS_UTIL_V2_PKG","schemaName":"SCH_RMAIS_DEV","sxml":""}