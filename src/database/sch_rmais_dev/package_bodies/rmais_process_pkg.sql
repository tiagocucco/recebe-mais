create or replace package body rmais_process_pkg as
    --
    procedure ins_log (
        p_msg varchar2
    ) is
    ...
    begin
        ...
        l_body := '{"cnpj_fornecedor" : "'
                  ...
                  || '"}';
        --
        l_response := json_value(rmais_process_pkg.get_response(l_url, l_body),
           '$.DATA.ITEM_ORACLE');
        --
        return l_response;
        --
    ...
    begin
        ...
        if p_po.count = 0 then
            --
            r_issuer.info := rmais_process_pkg.get_taxpayer(p_fornec, 'ISSUER');
            r_issuer.docs := rmais_process_pkg.get_po_list(l_body_po);
            r_issuer.cnpj := p_fornec;
            r_issuer.receiver := p_receiv;
            --
            rmais_process_pkg.ins_issuer(r_issuer);
            --
            setpo;
            --
        end if;
  ...
  --
    ...
    --
      ...
      --
            r_issuer.info := rmais_process_pkg.get_taxpayer(p_fornec, 'ISSUER');
      --
            l_transaction_id := rmais_process_pkg.get_po_list_v2(l_body_po);
      --
            begin
        --
    ...
    --    
        ...
        --
          ...
          --    
           ...
           --
            ...
            --
            --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Não foi possível identificar User Defined Fiscal Classification com parâmetros ('||l_define_descr||'), faça a escolha manual.'||'<br>';
            --
                            print('End Exception');
                    end;
            --
      ...
      --
        ...
        --
          ...
          -- 
          --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Identificado redução de base de INSS, utilizando : '||p_efd_lines.Intended_Use||'<br>';
          -- 
                    select distinct
                        1
                    into l_aux2
          ...
          --
                        ...
                        , 'Erro');
          --
          --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Não foi possível identificar utilização de INSS com base reduzida, faça a escolha manual.'||'<br>';
          --  
                end;
  ...
  --
    ...
    */
    -- Adicionado por Robson em 24/01/2023
        r$po := rmais_process_pkg.cursor_po_line(p_transaction_id => p_transaction_id);
    --
    ...
    -- 
      ...
      --
            log_efd('Ordem de Compra '
                    ...
                    p_efd.rhea.efd_header_id);
      --
      --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Ordem de Compra '||rLin.source_doc_number||' selecionada automaticamente pelo sistema.'||'<br>';
      --
         ...
         --
                log_efd('Atenção! Dados de entrega não localizado para Ordem de Compra '
                        ...
                        rlin.efd_line_id,
                        p_efd.rhea.efd_header_id);
         --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Atenção! Dados de entrega não localizado para Ordem de Compra '||rLin.source_doc_number||' selecionada.'||'<br>';
         --
            end if;
      --
            p_efd.rhea := rhea;
        ...
        --
          ...
          --
                    log_efd('PO '
                            ...
                            p_efd.rlin(p_idx).rlin.efd_line_id,
                            p_efd.rhea.efd_header_id);
          --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' PO '||p_Efd.rLin(p_Idx).rLin.source_doc_number||' Não localizada.'||'<br>';
          --
                end if;
       --
        ...
        --
                log_efd('Mais de uma Ordem de Compra encontrada. Favor selecionar manualmente.',
                        rlin.efd_line_id,
                        p_efd.rhea.efd_header_id);
        --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Mais de uma Ordem de Compra encontrada. Favor selecionar manualmente.'||'<br>';
        --
                g_po_find := true;
        --
            end if;
    ...
    --
        for r in (
            with tp_cfop as (
                ...
                select
                    case
                        when sum(nvl(a.line_amount, a.line_quantity * a.unit_price))
                ...
                from
                    ...
                    rmais_efd_headers   b,
                    tp_cfop             c,
                    rmais_inv_pay_group d
                where
                        a.efd_header_id = p_header_id -- 522741
                    and a.efd_header_id = b.efd_header_id
                    and a.efd_header_id = c.efd_header_id (+)
                    and a.item_code = d.item (+)
            ), tp_lines_a as (
                select
                    case
                        when b.lh = 1 then
                ...
                where
                    ( a.line_type_esp = 'Item'
                      or ( a.line_type_esp = 'Miscellaneous'
                ...
                from
                    (
                        select
                            ...
                            ,
                            nvl(l.line_amount_esp,
                                nvl(l.line_amount, l.line_quantity * l.unit_price))                                                                                   line_amount
                            ...
                            l.fiscal_classification_to
                               --, NVL(json_value(l.item_info,'$.DESCRIPTION'),l.item_descr_efd) item_desc
                               --, nvl(NVL(json_value(l.item_info,'$.DESCRIPTION'),l.item_descr_efd),json_value(l.order_info,'$.ITEM_DESCRIPTION')) item_desc Comentado por Robson 27/07/2023
                            ,
                            nvl(l.item_desc_esp,
                                nvl(
                                nvl(
                                ...
                                json_value(l.order_info, '$.ITEM_DESCRIPTION')
                            ))                                                                                                                                        item_desc -- Robson 27/07/2023
                               --, NVL(json_value(l.item_info,'$.ITEM_NUMBER'),l.item_code_efd)  item_code -- Comentado por Robson 27/07/2023
                            ,
                            case
                            ...
                            end                                                                                                                                       item_code -- Robson 27/07/2023
                               --, l.line_quantity -- Comentado por Robson 27/07/2023
                            ,
                            case
                                when not l.line_type_esp = 'Item' then
                                    ...
                                    l.line_quantity
                            end                                                                                                                                       line_quantity -- Robson 27/07/2023
                            ,
                            case
                                when l.line_type_esp = 'Item' then
                                    l.unit_price
                            ...
                            ,
                            json_value(l.shipto_info, '$.LOCATION_CODE')                                                                                              location_code
                            ,
                            json_value(l.order_info, '$.LINE_LOCATIONS.ASSESSABLE_VALUE')                                                                             location_name
                            ,
                            nvl(
                                json_value(l.order_info, '$.LINE_LOCATIONS.ASSESSABLE_VALUE'),
                                l.line_amount
                            )                                                                                                                                         line_amt
                            ,
                            nvl(
                                regexp_replace(
                                    json_value(l.order_info, '$.VENDOR_NAME'),
                                    '{|}',
                                    ''
                                ),
                                'Services'
                            ...
                            json_value(l.order_info, '$.VENDOR_NAME')                                                                                                 vendor_name
                            ,
                            nvl(
                                json_value(l.order_info, '$.LINE_LOCATIONS.SHIPMENT_NUM'),
                                1
                            )                                                                                                                                         shipment_num
                            ,
                            ...
                            nvl(
                                ...
                                rmais_process_pkg.get_parameter('DETERM_FACTOR')
                            )                                                                                                                                         determining_factor
                            ,
                            l.catalog_code_ncm,
                            l.cfop_to,
                        ...
                        group by
                            ...
                            l.line_quantity,
                            l.unit_price,
                            json_value(l.shipto_info, '$.LOCATION_CODE'),
                            json_value(l.order_info, '$.LINE_LOCATIONS.ASSESSABLE_VALUE'),
                            nvl(
                                json_value(l.order_info, '$.LINE_LOCATIONS.ASSESSABLE_VALUE'),
                                l.line_amount
            ...
            select
                    json_object(
                        'InvoiceNumber' is h.document_number,
                                ...
                                'InvoiceAmount' is j.line_amount_sum --h.total_amount
                                ,
                                'InvoiceDate' is to_char(h.issue_date, 'RRRR-MM-DD')
           --,'BusinessUnit'                        IS nvl(json_value(h.receiver_info,'$.DATA.BU_NAME'), h.receiver_name)
                                ,
                                'BusinessUnit' is nvl(
                            nvl(
                                json_value(h.receiver_info, '$.DATA.BU_NAME'),
                                json_value(h.receiver_info, '$.BU_NAME')
                            ),
                            'Não localizada'
                        ) --HDI Seguros S.A. BU
           --,'ProcurementBU'                       IS nvl(nvl(json_value(h.receiver_info,'$.DATA.BU_NAME'),json_value(h.receiver_info,'$.BU_NAME')), 'Não localizada')
       --    ,'Supplier'                            IS coalesce(json_value(h.issuer_info,'$.DATA.PARTY_NAME'),(
                        ,
                                'Supplier' is nvl(h.party_name,
                                                  nvl(
                                                                          nvl((
                                                                              select
                                                                                  max(json_value(l.order_info, '$.VENDOR_NAME'))
                                                                              from
                                                                                  rmais_efd_lines l
                                                                              where
          ...
          -- ,'PaymentTerms'                        IS CASE WHEN nvl(h.document_type,'PO') = 'NA' THEN (SELECT NAME FROM rmais_terms_ws WHERE term_id = h.payment_term_id) ELSE json_value(h.term_info,'$.TERMS') END
         ...
         --  SELECT json_arrayagg(json_object (
          ...
          --,'PayGroup'                             IS CASE WHEN nvl(h.PAYMENT_INDICATOR,'N') = 'N' THEN '' ELSE nvl(rmais_process_pkg.Get_Parameter('GET_NO_PAY'),'NAO GERA PAGAMENTO') END --21_09_2021 adicionado nao gera pagamento
                                ,
...
--99	Outros
           ...
           --,'Quantity'                            IS CASE h.model WHEN '00' THEN null ELSE l.line_quantity END
                                                ...
                                                'UnitPrice' is l.unit_price --l.unit_price + round((nvl(l.FREIGHT_LINE_AMOUNT,0) /l.line_quantity),2)
                                                ,
                                                'ProductType' is 'Services',
                                                'PayGroup' is
                                            case
                                                when payg.item is not null then
                                                    payg.payment_group
                                            end -- Robson 04/09/2023 
           ...
           --, 'ProductFiscalClassificationType'    IS nvl(l.CATALOG_CODE_NCM,'LACLS_NCM_SERVICE_CODE')
                            ...
                            from
                                tp_lines_c          l,
                                rmais_inv_pay_group payg
                            where
                                    l.efd_header_id = h.efd_header_id
                                and l.item_code = payg.item(+)
                        ),
             ...
             --'DueDate'                    IS to_char(nvl(first_due_date,sysdate),'RRRR-MM-DD'),--json_value(l2.order_info,'$.LINE_LOCATIONS.NEED_BY_DATE'),--to_char(first_due_date,'RRRR-MM-DD'),
                                            ...
                                            where
                                                ...
                                                'GrossAmount' is j.line_amount_sum, --h.total_amount,
             ...
             --'FileName'                   IS AT.filename,
                                            'FileName' is rmais_process_pkg.return_filename_croped(at.filename),--substr(SUBSTR(upper(AT.filename),1,INSTR(upper(AT.filename),'.PDF')-1),1,50)||SUBSTR(upper(AT.filename),INSTR(upper(AT.filename),'.PDF'),4),
             --'Title'                      IS  replace(replace(AT.filename,'.pdf',''),'.PDF',''),
                                            'Title' is rmais_process_pkg.return_filename_croped(at.filename, 'N'),--replace(replace(substr(SUBSTR(upper(AT.filename),1,INSTR(upper(AT.filename),'.PDF')-1),1,50)||SUBSTR(upper(AT.filename),INSTR(upper(AT.filename),'.PDF'),4),'.pdf',''),'.PDF',''),
             ...
             --'Description'                IS replace(replace(AT.filename,'.pdf',''),'.PDF',''),
                                            'Description' is rmais_process_pkg.return_filename_croped(at.filename, 'N'),--replace(replace(substr(SUBSTR(upper(AT.filename),1,INSTR(upper(AT.filename),'.PDF')-1),1,50)||SUBSTR(upper(AT.filename),INSTR(upper(AT.filename),'.PDF'),4),'.pdf',''),'.PDF',''),
                                            'Category' is
                                        case
                                            when at.filename is not null then
                                                'From Supplier'
            ...
            from
                ...
                tp_lines_s        j
            where
                    h.efd_header_id = at.efd_header_id (+)
                and h.efd_header_id = f.efd_header_id (+)
                and h.efd_header_id = j.efd_header_id
                and h.efd_header_id = p_header_id
        ) loop
      --
                    ...
                    json_object(
                        'InvoiceNumber' is h.document_number,
                            ...
                            h.receiver_name
                                ...
                                'Supplier' is coalesce(
                            json_value(h.issuer_info, '$.DATA.PARTY_NAME'),
                            (
                                                   select
                                                       max(json_value(l.order_info, '$.VENDOR_NAME'))
                                                   from
                            ...
                            h.issuer_name
                                ...
                                'AccountingDate' is to_char(sysdate, 'RRRR-MM-DD'),
                                'PaymentTerms' is json_value(h.term_info, '$.TERMS')
           --,'TermsDate'                           IS to_char(h.issue_date + h.term_info.DUE_DAYS,'RRRR-MM-DD')
                                ,
                                'TermsDate' is to_char(h.issue_date, 'RRRR-MM-DD'),
                                'LegalEntityIdentifier' is h.receiver_document_number,
          ...
          --chamada para ws base64 de layout via xml
            ...
            --
                    declare
                        ...
                        l_url  varchar2(300) := rmais_process_pkg.get_parameter('URL_GET_PDF');
                                    
            --UTL_URL.Escape(rmais_process_pkg.Get_Parameter('URL_GENERATE_PFD','TEXT_VALUE'), True);--'http://150.230.68.115/luznfe/rest.php?class=getPDF&method=executar'; --parametrizar endereço
                    begin
                        l_body :=
                            json_object(
  ...
  --
            ...
            json_object(
                'invoice_num' value rmh.document_number,
                    ...
                    case
                        ...
                        'name_supplier' value(coalesce(
                    json_value(rmh.issuer_info, '$.DATA.PARTY_NAME'),
                    (
                                   select
                                       max(json_value(l.order_info, '$.VENDOR_NAME'))
                                   from
    ...
    --ge
      ...
      --
      --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Falha ao enviar boleto.'||'<br>';
      --
        else
      --
    ...
    --
    --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Falha geral ao enviar boleto. Erro: '||sqlerrm||'<br>';
    --
    end send_boleto;
  --
    ...
    --
      ...
      --
        ...
        --
          ...
          --
            ...
            --
              ...
              --
                ...
                --
                    ...
                    --
                    --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Enviado para ERP (AP)'||'<br>';
                    --
                                    declare
                                        l_status rmais_efd_headers.document_status%type;
                                    begin
                  ...
                  --
                    ...
                    --
                    --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||nvl(r.retorno,' Erro entre em contato com o administrador.')||'<br>';
                    --
                                    update rmais_efd_headers
                                    set
                                        document_status = 'E',
            ...
            --
               ...
               --
                ...
                --
                --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Erro ao enviar documento, contacte o adminitrador'||'<br>';
                --
                        update rmais_efd_headers
                        set
                            document_status = 'E',
          ...
          --
                rmais_process_pkg.set_workflow(p_header_id,
                                               g_log_workflow,
                                               nvl(
                                 v('APP_USER'),
                                 '-1'
  ...
  --
    function get_registrationid (
        ...
        l_url  varchar2(1000) := rmais_process_pkg.get_parameter('GET_TAX_REGISTRATION')
                                || '/';
        l_resp clob;
    begin
    --
        ...
        l_resp := rmais_process_pkg.get_response(l_url);
  ...
  --
    ...
    begin
        ...
        rmais_process_pkg.g_log_workflow := null;
    	--
        g_shipments.delete;
    	--
        g_po_find := false;
        ...
        for r in (
    	  ...
    	  -- Robson 31/05/2023 Start
            if length(r.issuer_document_number) = 14 then
                l_body_simp := rmais_process_pkg.get_response2('http://152.67.41.84:9000/api/consultas/v1/simplesNacional/' || r.issuer_document_number
                );
    	  ...
    	  -- Robson 22/03/2023 Start
            ...
            if l_new_doc_number is not null then
                ...
                if l_flag_log = 0 then
    				...
    				--RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Número do documento alterado pelo sistema. (Número Original: '||r.document_number||')'||'<br>';
                end if;
            ...
            if nvl(r_efd.rhea.source_type,
    			...
    			--
                l_transaction_id := rmais_process_pkg.set_transaction_po_arrays(r.issuer_document_number,
                                                                                get_bu_cnpj(r.receiver_document_number),
                                                                                l_transaction_id);
    			--
            end if;
    		--l_transaction_id := rmais_process_pkg.set_transaction_po_arrays(r.issuer_document_number,r.receiver_document_number,l_transaction_id);
    		--
    	...
    	--END IF;
    			  ...
    			  --
                    ...
                    then
                        ...
                        begin
    						...
    						--
                            l_transaction_id2 := rmais_process_pkg.get_itens(l_id,
                                                                             r_efd.rlin(ix_l).rlin.item_code_efd,
                                                                             '');
    						--
                    ...
                    if r_efd.rlin(ix_l).rlin.item_code_efd is null then
    					...
    					--
    					--RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Item não informado. Favor informar o item e revalidar documento.'||'<br>';
    					--            
    		...
    		--
                ...
                exception
                    when others then
    					...
    					--
    					--RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Falha ao atualizar linha...'||SQLERRM||'<br>';
    					--
                end;
    			--
                ...
                if nvl(r_efd.rlin(ix_l).rlin.status,
    				...
    				BEGIN
    				--
    				l_r := rmais_process_pkg.CHECK_TAX_LINE(r_Efd.rLin(ix_l).rLin.efd_line_id);
    				--
    				IF l_r = 'F' THEN
    				 --
            ...
            if r_efd.rhea.issuer_info not like '%PARTY_NAME%' then
                ...
                log_efd('Fornecedor não localizado no ERP, verificar cadastro Oracle.',
                        ...
                        r_efd.rhea.efd_header_id,
                        'ERRO');
    			--RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Fornecedor não localizado no ERP, verificar cadastro Oracle.'||'<br>';
                print('passou aqui');
    			--
            else
    			--
                        ...
                        exception
    						...
    						/*workflow*/
                                log_efd('Não foi possível buscar cadastro do fornecedor.',
                                        ...
                                        'ERRO');
    						--
    						--RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Não foi possível buscar cadastro do fornecedor.'||'<br>';
    						--
                        end;
    				    --
                end;
            ...
            exception
                when others then
                    ...
                    exception
    				  ...
    				  --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Falha ao atualizar Header '||SQLERRM||'<br>';
                    end;
            end;
    		--
    		...
    		--        
                teste_vx := 1;
                print('remain');
                rmais_process_pkg.set_workflow(r.efd_header_id,
                                               g_log_workflow,
                                               nvl(
                                 v('APP_USER'),
                                 '-1'
            ...
            begin
                ...
                print('Parametro: ' || nvl(
                    rmais_process_pkg.get_parameter('SEND_ERP_AUTO'),
                    '2'
                ));
                print('Status da linha: ' || get_status_lines(r_efd.rhea.efd_header_id));
                print('debug wkf antes de enviar');
                print('status => ' || nvl(r_efd.rhea.document_status,
                                          'I'));
                print('SEND_ERP_AUTO => ' || nvl(
                    rmais_process_pkg.get_parameter('SEND_ERP_AUTO'),
                    '2'
                ));
                print('AUTO VALID => ' || get_status_lines(r_efd.rhea.efd_header_id));
                print('SEND ERP => ' || nvl(p_send_erp, 'VAZIO'));
                if
                ...
                then --
                    ...
                    print('debug wft preparado para enviar.');
                    print('Passo2-> MODEL: '
                          || r.model
                          || ', define_det_entry_type: '
                    ...
                    print('debug gravar');
                    if g_log_workflow is null then
                        if r_efd.rhea.document_status = 'V' then
                            g_log_workflow := 'Documento Válido';
                        else
                            g_log_workflow := r_efd.rhea.document_status;
                        end if;
                    ...
                    commit;
                    rmais_process_pkg.set_workflow(l_header_workflow,
                                                   g_log_workflow,
                                                   nvl(
                                     v('APP_USER'),
                                     '-1'
...
--
  ...
  --
        print('l_body: ' || l_body);
  --
        l_response := rmais_process_pkg.get_response2(l_url, l_body, 'POST');
  --
  ...
  --
    ...
    --
    --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Não foi possível criar retenção, contacte o administrador.'||'<br>';
    --
...
--
    procedure update_invoice (
        p_header_id in number,
        p_att_bol   in number default null
    ) is

        l_url_mod    constant varchar2(1000) := rmais_process_pkg.get_parameter('GET_URL_UPDATE_MODEL');--'http://152.67.41.84:9000/api/payables/v2/updateInvoiceModel';
        l_url_att    constant varchar2(1000) := rmais_process_pkg.get_parameter('GET_URL_UPDATE_ATTACH');--'http://152.67.41.84:9000/api/payables/v2/updateInvoiceAttach';
    ...
    --
        with tp_cfop as (
            ...
            select
                ...
                replace(
                    replace(
                        replace(
                            replace(
                                xxrmais_util_pkg.base64encode(a.blob_file),
                                chr(10),
                                ''
                            ),
                            chr(13),
            ...
            from
                rmais_attachments_bol a
            where
                p_att_bol is not null
        ), tp_efd as (
            select
                ...
                nvl(
                    nvl(
                        json_value(a.receiver_info, '$.DATA.BU_NAME'),
                        json_value(a.receiver_info, '$.BU_NAME')
                    ),
                    'HDI Seguros S.A. BU'
                )                                                         businessunit,--'INATIVO-HDI Seguros S.A. BU' BusinessUnit,
                a.org_id,
                   -- case when a.org_id = 300000024270757 and a.issue_date <= to_date('2022-10-16','YYYY-MM-DD') then 'INATIVO-HDI Seguros S.A. BU' else nvl(nvl(json_value(A.RECEIVER_INFO,'$.DATA.BU_NAME'),json_value(a.receiver_info,'$.BU_NAME'))) end  BusinessUnit,--, 'HDI Seguros S.A. BU') BusinessUnit,--'INATIVO-HDI Seguros S.A. BU' BusinessUnit,
                   -- case when h.org_id = 300000024270757 and a.issue_date <= to_date('2022-10-16','YYYY-MM-DD') then 'INATIVO-HDI Seguros S.A. BU' else nvl(nvl(json_value(A.RECEIVER_INFO,'$.DATA.BU_NAME'),json_value(a.receiver_info,'$.BU_NAME'))) end  BusinessUnit,--, 'HDI Seguros S.A. BU') BusinessUnit,--'INATIVO-HDI Seguros S.A. BU' BusinessUnit,
                ...
                coalesce(
                    json_value(a.issuer_info, '$.DATA.PARTY_NAME'),
                    (
                        select
                            max(json_value(a.receiver_info, '$.BU_NAME'))
                        from
                            rmais_efd_lines b
                        where
                    ...
                    a.issuer_name
                )                                                         supplier, --'HI SERVICE TERCEIRIZACAO LTDA' Supplier,
                ...
                rmais_process_pkg.return_filename_croped(b.filename)      filename,
                rmais_process_pkg.return_filename_croped(b.filename, 'N') title,
                rmais_process_pkg.return_filename_croped(b.filename, 'N') description,
                case
    ...
    --
        if json_value(l_response, '$.retorno') = 'Nota Fiscal não localizada! ' then
            l_flag_falha := 3;
        elsif json_value(l_response, '$.retorno') != '201' then
            l_flag_falha := 2;
        elsif p_att_bol is not null then
            l_flag_falha := 0;
        else
        --
            print('l_body_doc: ' || l_body_doc);
        --
            l_response := rmais_process_pkg.get_response2(l_url_mod, l_body_doc, 'POST');
        --
    ...
    --
        commit;
    --
        if l_flag_falha = 0 then
            rmais_process_pkg.g_log_workflow := rmais_process_pkg.g_log_workflow
                                                || ' Update Nota'
                                                ||
                case
                    when p_att_bol is not null then
                        ' (boleto)'
                end
                                                || '<br>';
        elsif l_flag_falha = 1 then
            log_efd('Erro ao atualizar informações adicionais. ('
                    ...
                    p_header_id,
                    'Erro');
        --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Erro ao atualizar informações adicionais. ('||JSON_VALUE(l_response,'$.retorno')||')'||'<br>';
            ...
            log_efd('Erro ao atualizar anexo do documento. ('
                    ...
                    p_header_id,
                    'Erro');
        --RMAIS_PROCESS_PKG.g_log_workflow := RMAIS_PROCESS_PKG.g_log_workflow ||' Erro ao atualizar anexo do documento. ('||JSON_VALUE(l_response,'$.retorno')||')'||'<br>';
        elsif l_flag_falha = 3 then
            rmais_process_pkg.g_log_workflow := rmais_process_pkg.g_log_workflow
                                                || ' Nota não localizada para Update.'
                                                || '<br>';
        end if;
    --
...
--
    ...
    begin
        ...
        --rmais_global_pkg.g_enable_log := 'Y';
            ...
            rmais_process_pkg.update_invoice(p_header_id => c.efd_header_id);
        end loop;
...
--
    function cancel_nf_erp (
        ...
        l_url_mod  constant varchar2(1000) := rmais_process_pkg.get_parameter('GET_URL_CANCEL_INVOICE');--'http://152.67.41.84:9000/api/payables/v2/cancelInvoiceService';
...
--
    ...
    --
        commit;
    --
        rmais_process_pkg.set_workflow(p_efd_header_id,
                                       'Nota reprocessada',
                                       nvl(
                         v('APP_USER'),
                         '-1'
    ...
    begin
        for rw in (
            ...
            begin
                ...
                rmais_process_pkg.update_devolucao(p_header_id, p_header_id_anexo, rw.id_nd);
            exception
                when no_data_found then
                    update rmais_efd_headers
                    set
...
--
  ...
  --
    ...
    --
end rmais_process_pkg;
/


-- sqlcl_snapshot {"hash":"e52bdfb00d18618eabd19967475053aad45c1e34","type":"PACKAGE_BODY","name":"RMAIS_PROCESS_PKG","schemaName":"SCH_RMAIS_DEV","sxml":""}